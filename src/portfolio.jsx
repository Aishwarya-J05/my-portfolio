...
const GlobalStyles = () => (
  <style>{`
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    @media(max-width:768px) {
      .chip-desktop-grid { display:none !important; }
      .chip-mobile-scroll { display:flex !important; }
    }
  ...
  return (
    <section id="skills" style={{ padding:"160px 64px", position:"relative" }}>
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        ...
        <div style={{ overflowX:"auto", WebkitOverflowScrolling:"touch", marginBottom:48, paddingBottom:4 }}>
          <div style={{ display:"flex", gap:10, flexWrap:"nowrap", width:"max-content" }}>
            {STACK_GROUPS.map(g=>(
              <button key={g.id} onClick={()=>setActiveTab(g.id)}
                style={{
                  padding:"10px 22px", borderRadius:100, fontSize:12, fontWeight:700,
                  letterSpacing:1, textTransform:"uppercase", cursor:"pointer",
                  fontFamily:"var(--font-head)", border:"none", whiteSpace:"nowrap", flexShrink:0,
        ...
        <div key={activeTab} style={{
          ...
          {/* Desktop: 3-col grid. Mobile: horizontal scroll row */}
          <div className="chip-desktop-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
            {active.skills.map((s,i)=>(
              <SkillChip key={s.name} {...s} accent={active.accent} delay={i*60} />
            ))}
          </div>
          {/* Mobile horizontal scroll */}
          <div className="chip-mobile-scroll" style={{ display:"none", overflowX:"auto", WebkitOverflowScrolling:"touch", gap:12, paddingBottom:8 }}>
            {active.skills.map((s,i)=>(
              <div key={s.name} style={{ flexShrink:0, width:200 }}>
                <SkillChip {...s} accent={active.accent} delay={0} />
              </div>
            ))}
          </div>
        </div>
...
function ExperienceSection() {
  ...
  const [expandedIdx, setExpandedIdx] = useState(0); // mobile accordion

  const toggleMobile = (i) => setExpandedIdx(expandedIdx === i ? -1 : i);

  const DetailPanel = ({ e }) => (
    <div style={{
      background:`linear-gradient(135deg,${e.color}14,rgba(255,255,255,0.02))`,
      border:`1px solid ${e.color}33`,
      borderRadius:20, padding:24, backdropFilter:"blur(24px)",
      animation:"fadeSlideIn 0.35s cubic-bezier(0.16,1,0.3,1)",
    }}>
      <div style={{ display:"inline-flex",alignItems:"center",gap:8,padding:"5px 14px",borderRadius:100,background:`${e.color}18`,border:`1px solid ${e.color}33`,marginBottom:16 }}>
        <div style={{ width:6,height:6,borderRadius:"50%",background:e.color }}/>
        <span style={{ fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:e.color }}>
          {e.type}{e.dateRange ? ` · ${e.dateRange}` : ""}
        </span>
      </div>
      <h3 style={{ fontFamily:"var(--font-head)",fontSize:"1.2rem",fontWeight:800,lineHeight:1.2,marginBottom:6,color:"white" }}>{e.role}</h3>
      <p style={{ fontSize:13,color:e.color,fontWeight:600,marginBottom:16 }}>{e.co}</p>
      <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
        {e.bullets.map((b,j)=>(
          <div key={j} style={{ display:"flex",gap:12,alignItems:"flex-start",padding:"12px 16px",background:"rgba(255,255,255,0.03)",borderRadius:12,border:"1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ width:5,height:5,borderRadius:"50%",background:e.color,flexShrink:0,marginTop:6,boxShadow:`0 0 6px ${e.color}` }}/>
            <p style={{ fontSize:13,color:"rgba(255,255,255,0.65)",lineHeight:1.75,fontWeight:300 }}>{b}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <section id="experience" style={{ padding:"160px 64px",position:"relative",background:"rgba(255,255,255,0.01)",borderTop:"1px solid rgba(255,255,255,0.03)" }}>
      <div style={{ maxWidth:1100,margin:"0 auto" }}>
        ...
        {/* ── DESKTOP: two-column ── */}
        <div className="exp-desktop" style={{ display:"grid",gridTemplateColumns:"1fr 1.4fr",gap:48,alignItems:"start" }}>
          {/* Left list */}
            ...
            {EXPERIENCES.map((e,i)=>(
              <Reveal key={i} delay={i*80}>
                <div onClick={()=>setOpenIdx(i)}
                  ...
                  {openIdx===i && <div style={{ position:"absolute",left:0,top:"20%",bottom:"20%",width:3,borderRadius:100,background:e.color,boxShadow:`0 0 12px ${e.color}88` }}/>}
                  <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:6 }}>
                    <span style={{ fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:e.color,padding:"3px 10px",borderRadius:100,background:`${e.color}18`,border:`1px solid ${e.color}33` }}>{e.type}</span>
                    {e.dateRange && <span style={{ fontSize:11,color:"rgba(255,255,255,0.38)",letterSpacing:0.5,fontWeight:500 }}>{e.dateRange}</span>}
                  </div>
          ...
          {/* Right detail */}
          <div key={openIdx} style={{ background:`linear-gradient(135deg,${EXPERIENCES[openIdx].color}14,rgba(255,255,255,0.02))`,border:`1px solid ${EXPERIENCES[openIdx].color}33`,borderRadius:28,padding:44,backdropFilter:"blur(24px)",animation:"fadeSlideIn 0.5s cubic-bezier(0.16,1,0.3,1)",position:"sticky",top:120 }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
              {EXPERIENCES[openIdx].bullets.map((b,j)=>(
                <div key={j} style={{ display:"flex",gap:16,alignItems:"flex-start",padding:"16px 20px",background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.06)",animation:`fadeSlideIn 0.5s cubic-bezier(0.16,1,0.3,1) ${j*80}ms both` }}>
        ...
        {/* ── MOBILE: accordion ── */}
        <div className="exp-mobile" style={{ display:"none", flexDirection:"column", gap:8 }}>
          {EXPERIENCES.map((e,i)=>(
            <div key={i}>
              {/* Header tap */}
              <div onClick={()=>toggleMobile(i)}
                style={{
                  padding:"20px 22px", borderRadius:16, cursor:"pointer", position:"relative",
                  background: expandedIdx===i ? `linear-gradient(135deg,${e.color}18,rgba(255,255,255,0.04))` : "rgba(255,255,255,0.02)",
                  border: expandedIdx===i ? `1px solid ${e.color}44` : "1px solid rgba(255,255,255,0.06)",
                  transition:"all 0.3s cubic-bezier(0.23,1,0.32,1)",
                  display:"flex", alignItems:"center", justifyContent:"space-between",
                }}>
                {expandedIdx===i && <div style={{ position:"absolute",left:0,top:"20%",bottom:"20%",width:3,borderRadius:100,background:e.color,boxShadow:`0 0 10px ${e.color}88` }}/>}
                <div style={{ paddingLeft:8 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:4 }}>
                    <span style={{ fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:e.color,padding:"2px 8px",borderRadius:100,background:`${e.color}18`,border:`1px solid ${e.color}33` }}>{e.type}</span>
                    {e.dateRange && <span style={{ fontSize:10,color:"rgba(255,255,255,0.38)" }}>{e.dateRange}</span>}
                  </div>
                  <h4 style={{ fontFamily:"var(--font-head)",fontSize:16,fontWeight:700,color:expandedIdx===i?"white":"rgba(255,255,255,0.7)" }}>{e.role}</h4>
                  <p style={{ fontSize:12,color:expandedIdx===i?e.color:"rgba(255,255,255,0.35)",fontWeight:500,marginTop:2 }}>{e.co}</p>
                </div>
                {/* Chevron */}
                <div style={{ fontSize:18, color:"rgba(255,255,255,0.4)", transition:"transform 0.3s", transform: expandedIdx===i ? "rotate(180deg)" : "rotate(0deg)", flexShrink:0, marginLeft:12 }}>⌄</div>
              </div>

              {/* Expanded detail — inline, pure React, no CSS tricks */}
              {expandedIdx===i && (
